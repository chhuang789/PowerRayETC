...
#   v1.09 Optimal the performance for each worker scanning
#       A. Change all /etc/config/cgminer scripts in one scrip "Build Version=20221105-004"
...
POWERRAY_ETC_VERSION = 1.09
POWERRAY_ETC_BUILD_VERSION = "Build Version=20221105-005"
...
def onScan():
    ...
    for i in range(len(workerArray)):
        ...
        def getRemoteScript(client, scriptfile):
            ...
            for line in lines:
                if line[0] == "'":
                    ...
                    logger.info(scriptfile + " -> " + line.rstrip())
            strReturn = lines
            if strReturn == '':
                strReturn = 'NA'
        ...
        ''' Replace the following lines by one script "Build Version=20221105-005"
        ...
        '''
        # Get info from /etc/config/system
        strTmps, strFailDesc = getRemoteScript(client, "getEtcConfigCgminer.sh")
        if strFailDesc != '':
            workerArray[i].FailDesc = strFailDesc
            continue
        if str(type(strTmps)) == "<class 'list'>":
            if strTmps[0][0] == "'":
                workerArray[i].PoolAddress = strTmps[0][1:-2]
            else:
                workerArray[i].PoolAddress = strTmps[0].rstrip()
            if strTmps[1][0] == "'":
                workerArray[i].WalletAddress = strTmps[1][1:-2]
            else:
                workerArray[i].WalletAddress = strTmps[1].rstrip()
            if strTmps[2][0] == "'":
                workerArray[i].Password = strTmps[2][1:-2]
            else:
                workerArray[i].Password = strTmps[2].rstrip()
            workerArray[i].Account = 'NA'  # No account for ETC
            if strTmps[3][0] == "'":
                workerArray[i].MinerOPFreq = strTmps[3][1:-2]
            else:
                workerArray[i].MinerOPFreq = strTmps[3].rstrip()
            if strTmps[4][0] == "'":
                workerArray[i].Hostname = strTmps[4][1:-2]
            else:
                workerArray[i].Hostname = strTmps[4].rstrip()
        logger.info("End of %s", "getEtcConfigCgminer.sh")
        ...
        if (str(type(strTmp) == "<class 'list'>")):
            if (len(strTmp) == 1):
                if strTmp[0] == '':
                    strTemp = 'NA'
                if strTmp[0][0] == "'":
                    strTemp = strTmp[0][1:-2]
                else:
                    strTemp = strTmp[0].rstrip()
        else:
            strTemp = 'NA'
        workerArray[i].PowerConsumption = strTemp
        if strFailDesc.find("I2C_SMBUS") >= 0:
            workerArray[i].FailCode = 7
            workerArray[i].FailDesc = strFailDesc
...
############################################################################################################################################################################
    ...
    if FHLog != None:
        FHLog.setFormatter(logging.Formatter(strLogFormater2))
    logger.warning(
        "====== End of Worker scanning, please check the UI of scan result.")
    if bLevelname:
        logger.handlers[0].setFormatter(logging.Formatter(strLogFormater1))
    if FHLog != None:
        FHLog.setFormatter(logging.Formatter(strLogFormater1))
